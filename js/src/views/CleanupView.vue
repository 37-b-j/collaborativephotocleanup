...
// Cache for already-loaded original images (fileId -> blob Object URL)
// Blob URLs persist in memory even when <img> is removed from DOM
const origImageCache = new Map<number, string>()

function isOrigCached(fileId: number): boolean {
  return origImageCache.has(fileId)
}

function getCachedOrigUrl(fileId: number): string | undefined {
  return origImageCache.get(fileId)
}

function markOrigCached(fileId: number, blobUrl: string): void {
  // Revoke previous blob URL if exists (avoid memory leak)
  const prev = origImageCache.get(fileId)
  if (prev) URL.revokeObjectURL(prev)
  origImageCache.set(fileId, blobUrl)
}

function clearOrigCache(): void {
  origImageCache.forEach(url => URL.revokeObjectURL(url))
  origImageCache.clear()
}
...
function onDetailImgLoaded() {
  ...
  if (!fileId || !origUrl) return

  // If already cached (blob URL is showing), nothing to do
  if (isOrigCached(fileId)) return

  // Preview just loaded — fetch original as blob for persistent caching
  const loadingId = fileId
  fetch(origUrl)
    .then(r => r.blob())
    .then(blob => {
      const blobUrl = URL.createObjectURL(blob)
      markOrigCached(loadingId, blobUrl)
      if (detailImage.value?.fileId === loadingId) {
        detailHiresUrl.value = blobUrl
...
function openDetail(img: ClusterImage) {
  ...
  const origUrl = generateUrl("api/v1/preview/" + img.fileId)
  const cached = getCachedOrigUrl(img.fileId)
  if (cached) {
    detailHiresUrl.value = cached
    detailImgLoading.value = false
  } else {
    detailHiresUrl.value = ""
    detailImgLoading.value = true
  }
  detailImage.value = { ...img, _origUrl: origUrl }
}
...
function onSwipeImgLoaded(e: Event) {
  ...
  const origUrl = swipeDetailImage.value?._origUrl
  const currentId = swipeDetailImage.value?.id
  if (!origUrl || !currentId) return

  // If already cached (blob URL is showing), nothing to do
  if (isOrigCached(currentId)) return
  ...
  fetch(origUrl)
    ...
    .then(blob => {
      const blobUrl = URL.createObjectURL(blob)
      markOrigCached(currentId, blobUrl)
      if (swipeDetailImage.value?.id === currentId) {
        swipeHiresUrl.value = blobUrl
      }
    })
    .catch(() => {})
}
...
function openSwipeDetail(img, idx) {
  ...
  const origUrl = generateUrl("api/v1/preview/" + img.id)
  swipeHiresUrl.value = getCachedOrigUrl(img.id) || ""
  swipeDetailImage.value = { ...img, _origUrl: origUrl }
...
function swipeDetailPrev() {
  ...
  const origUrl = generateUrl("api/v1/preview/" + p.id)
  swipeHiresUrl.value = getCachedOrigUrl(p.id) || ""
  swipeDetailImage.value = { ...p, _origUrl: origUrl }
}
...
function detailPrev() {
  ...
  const origUrl = generateUrl("api/v1/preview/" + p.fileId)
  const cached = getCachedOrigUrl(p.fileId)
  if (cached) {
    detailHiresUrl.value = cached
    detailImgLoading.value = false
  } else {
  ...
  detailImage.value = { ...p, _origUrl: origUrl }
}
